function resizeMenu() {
    ...
    if ($(this).hasClass("btn-prev")) {
        ...
        for (
            let i = prevItemIndex; i >= 0 && i > prevItemIndex - itemsToScroll; i--
        )
            prevItemsWidth -=
        ...
        $("#horizontal-nav .menu-wrap .menu").css({
            left: newScrollX
        });
        ...
        for (
            let i = nextItemIndex + 1; i < nextItemIndex + itemsToScroll &&
            nextItemIndex + itemsToScroll <= $("#horizontal-nav .list-item").length; i++
        )
            prevItemsWidth +=
            $(`#horizontal-nav .list-item:eq(${i})`).outerWidth() + 14;
